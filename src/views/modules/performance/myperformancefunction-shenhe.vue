  ...
  <el-dialog
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="visible = false">取消</el-button>
       <el-button type="primary" v-if="isAuth('sys:performancefunction:shenhe') " @click="dataFormSubmit()">审核通过</el-button>
       <el-button type="primary" v-else-if="isAuth('sys:performancefunction:rlshenhe') " @click="rlSubmit()">人力审核通过</el-button>
       <el-button type="primary" v-else-if="isAuth('sys:performancefunction:yuanshenhe') " @click="yuanSubmit()">院审核通过</el-button>
      <el-button type="primary" v-else @click="dataFormSubmit()">确定</el-button>

      <el-button type="danger" v-if="isAuth('sys:performancefunction:shenhe') && dataForm.status!=3" @click="bohuiSubmit()">驳回</el-button>
    </span>
  </el-dialog>
</template>
    ...
    methods: {
      ...
      dataFormSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sys/performancefunction/shenhe`),
              method: 'post',
      ...
      rlSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sys/performancefunction/rlshenhe`),
              method: 'post',
              data: this.$http.adornData({
                'id': this.dataForm.id || undefined,
                'userId': this.dataForm.userId,
      ...
      yuanSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sys/performancefunction/yuanshenhe`),
              method: 'post',
              data: this.$http.adornData({
                'id': this.dataForm.id || undefined,
                'userId': this.dataForm.userId,
