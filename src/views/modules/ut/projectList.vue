<template>
  <div class="mod-user">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <!-- <el-form-item>
         <el-date-picker v-model="dataForm.createMonth" type="month" value-format="yyyy-MM"
      ...
      <!--<el-form-item>
        <el-date-picker v-model="dataForm.excelMonth" type="month" value-format="yyyy-MM"
                        placeholder="选择上传月份" style="width: 150px" :clearable="false">
        </el-date-picker>
    ...
    <el-table
      ...
      <el-table-column label="启动时间" prop="startTime" :formatter="formatDate" header-align="center" width="95"
                       align="center"></el-table-column>
      <el-table-column label="完成时间" prop="endTime" :formatter="formatDate" width="95" header-align="center"
                       align="center"></el-table-column>
      ...
      <!--<el-table-column label="项目类型" prop="projectType" header-align="center" align="center"></el-table-column>-->
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="50">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="updateHandle(scope.row.id,scope.row.code,
        scope.row.name,scope.row.ut)">修改
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 弹窗,  修改 上传附件-->
    <project-update ref="projectUpdate" @refreshDataList="getDataList"></project-update>
    <upload ref="upload" @refreshDataList="getDataList"></upload>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          ...
          data: this.$http.adornData({
            ...
            'size': this.pageSize
            // 'excelMonth': this.dataForm.excelMonth
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
      ...
      // 上传
      uploadHandle () {
        /* if (this.dataForm.excelMonth === '') {
          this.$message.info('请选择项目月份')
          return
        } */
        this.uploadVisible = true
        // this.$refs.upload.init(this.dataForm.excelMonth)
        this.$refs.upload.init()
      },
      // 修改
      updateHandle (id, code, name, ut) {
        this.$refs.projectUpdate.init(id, code, name, ut)
      }
    }
  }
</script>
